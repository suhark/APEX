...
function buildRows(ticks: Tick[]) {
  ...
  return durations.map((duration, index) => {
    ...
    return {
      ...
      duration,
      duration_unit: 't',
      score: Math.min(100, Math.round(quality * 6 + (fast !== null && slow !== null ? 20 : 0) + Math.min(20, movement * 5000))),
      estimated_probability: probability,
      confidence_lower: Math.max(0.3, probability - (ticks.length >= 100 ? 0.04 : 0.12)),
      break_even_probability: breakEven,
      edge,
      status: ticks.length < 100 ? 'NO SIGNAL' : edge >= 0.05 ? 'QUALIFIED' : edge > 0 ? 'WATCH' : 'NO SIGNAL',
      sample_size: ticks.length,
      breakdown: { technical: fast !== null && slow !== null ? 18 : 0, statistical: 0, validation: 0, economics: Math.round(edge * 100), data: quality },
      model_version: 'v1-ema9-20',
      validation_version: 'unvalidated',
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!configuredKey || suppliedKey !== configuredKey) {
    console.error('[scanner-run] Unauthorized request', { has_configured_key: Boolean(configuredKey), has_supplied_key: Boolean(suppliedKey), method: req.method });
    return res.status(401).json({ error: 'Unauthorized scanner run. Send Authorization: Bearer <SCANNER_INGEST_KEY>.' });
  }
  ...
  try {
    ...
    if (!url || !serviceKey) {
      console.error('[scanner-run] Missing Supabase credentials', { has_url: Boolean(url), has_service_key: Boolean(serviceKey) });
    ...
    if (error) { console.error('[scanner-run] Upsert failed', { message: error.message, code: error.code, details: error.details, hint: error.hint }); return res.status(502).json({ error: 'Could not persist scanner run.', detail: error.message, code: error.code }); }
    return res.status(200).json({ ok: true, version: 'v1-live-persisted', sample_size: rows[0]?.sample_size ?? 0, rows: data ?? [] });
  } catch (error) {
    console.error('[scanner-run]', error);
    return res.status(502).json({ error: error instanceof Error ? error.message : 'Scanner run failed.' });
  }
}
