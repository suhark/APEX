...
function App() {
  ...
  const liveMode = deriv.authState === 'connected' && deriv.account !== null;
  ...
  return <div className="app-shell">
    <aside className={mobileNav ? 'sidebar open' : 'sidebar'}>
      ...
      <div className="workspace-chip"><span className="live-dot" /> {liveMode ? 'Deriv connected' : 'Synthetic workspace'} <b>{liveMode ? deriv.account?.is_virtual ? 'DEMO' : 'LIVE' : 'DEMO'}</b></div>
...
function PageView({ page, workspace, bots, trades, tick, runTrade, toggleBot, updateWorkspace, setPage, setNotice, deriv, onDerivConnect, onDerivDisconnect, liveMode }: { page: Page; workspace: Workspace | null; bots: BotRow[]; trades: Trade[]; tick: number; runTrade: (details: { instrument: string; direction: string; stake: number; source: string; botName?: string }) => Promise<void>; toggleBot: (bot: BotRow) => Promise<void>; updateWorkspace: (changes: Partial<Workspace>) => Promise<void>; setPage: (page: Page) => void; setNotice: (notice: string) => void; deriv: ReturnType<typeof useDerivConnection>; onDerivConnect: (token: string, appId: string) => void; onDerivDisconnect: () => void; liveMode: boolean }) {
  ...
  return <Settings workspace={workspace} updateWorkspace={updateWorkspace} setNotice={setNotice} deriv={deriv} onDerivConnect={onDerivConnect} onDerivDisconnect={onDerivDisconnect} liveMode={liveMode} />;
}
...
function Dashboard({ workspace, bots, trades, tick, toggleBot, setPage, liveMode, derivAccount }: { workspace: Workspace; bots: BotRow[]; trades: Trade[]; tick: number; toggleBot: (bot: BotRow) => Promise<void>; setPage: (page: Page) => void; liveMode: boolean; derivAccount: { loginid: string; balance: number; is_virtual: boolean } | null }) {
