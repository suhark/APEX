...
import { getTicksHistory, subscribeTicks, symbolMap, type DerivTick } from './deriv-client';
import { Activity, BarChart3, Calculator, CandlestickChart, CheckCircle2, Gauge, LineChart, RefreshCw, ShieldCheck, Target, TrendingDown, TrendingUp, Layers } from 'lucide-react';
...
export function AnalysisLab() {
  ...
  useEffect(() => {
    const code = (symbolMap[symbol] ?? 'R_75') as any;
    let cancelled = false;
    setLoading(true); setConnected(false); setFeedError(''); setTicks([]);
    getTicksHistory(code, 300).then((history) => {
      if (cancelled) return;
      if (!history.prices.length) throw new Error('No historical ticks were returned.');
      setTicks(history.prices.map((quote, index) => ({ quote, time: (history.times[index] ?? Math.floor(Date.now() / 1000)) * 1000 })));
      setLoading(false);
    }).catch((error) => { if (!cancelled) { setFeedError(error instanceof Error ? error.message : 'Unable to load Deriv history. Connect Deriv and try again.'); setLoading(false); } });
    const unsubscribe = subscribeTicks(code, (tick: DerivTick) => { if (cancelled) return; setConnected(true); setLoading(false); setFeedError(''); setTicks((current) => [...current.slice(-399), { quote: tick.quote, time: tick.epoch * 1000 }]); });
